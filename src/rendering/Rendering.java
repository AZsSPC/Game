...
/*
 public static void RenderGround(){  try{
 ...
 public static void RenderAll(){  try{
 ...
 AllGround)))))));  }catch(IOException iOE){  }  }
 ...
 tpx *= ImageS;  tpy *= ImageS;  texture += "";  try{
 BufferedImage CurrentImage = updateToCof(ImageIO.read(new File(texture)));
 for(int x = 0; x < ImageS; x++){
 for(int y = 0; y < ImageS; y++){
 ReturnImage.setRGB(tpx + x, tpy + y, CurrentImage.getRGB(x, y));  }  }  }catch(IOException iOException){
 BufferedImage CurrentImage = updateToCof(ImageIO.read(new File(IW("null"))));
 for(int x = 0; x < ImageS; x++){
 for(int y = 0; y < ImageS; y++){
 ReturnImage.setRGB(tpx + x, tpy + y, CurrentImage.getRGB(x, y));  }  }  }  return ReturnImage;  }
 ...
 int W = Uat.getWidth();  int H = Uat.getHeight();  x -= W / 2;  y -= H / 2;  int Yy = y;
 for(int X = 0; X < W; X++, x++){
 for(int Y = 0; Y < H; Y++, y++){  try{
 if(Uat.getRGB(X, Y) != new Color(2, 1, 8).getRGB()){
 Base.setRGB(x, y, Uat.getRGB(X, Y));  }  }catch(Exception e){  }  }
 y = Yy;  }  return Base;  }

 private static BufferedImage updateUatSize(int size, BufferedImage InputImage){
 int W = InputImage.getWidth();  int H = InputImage.getHeight();
 BufferedImage CurrentImage = new BufferedImage(W * (size + 1), H * (size + 1), 1);
 for(int x = 0; x < W * (size + 1); x++){
 for(int y = 0; y < H * (size + 1); y++){
 CurrentImage.setRGB(x, y, InputImage.getRGB(x / (size + 1), y / (size + 1)));  }  }  return CurrentImage;  }
 ...
 for(int x = 0; x < ImageS; x++){  for(int y = 0; y < ImageS; y++){
 CurrentImage.setRGB(x, y, InputImage.getRGB(x / (Cof + 1), y / (Cof + 1)));  }  }  return CurrentImage;  }
 ...
 for(int Xx = x; Xx < fx; Xx++){  for(int Yy = y; Yy < fy; Yy++){
 CurrentImage.setRGB(Xx - x, Yy - y, InputImage.getRGB(Xx, Yy));  }  }  return CurrentImage;  }
 */
